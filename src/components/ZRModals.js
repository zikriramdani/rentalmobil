...
function ZRModals(props) {
  ...
  return (
    <Fragment>
      <div
        className={`modal fade ${onShow ? 'show' : ''}`}
        style={{ display: onShow ? 'block' : '' }}
        tabIndex="-1"
      >
        <div
          className={`modal-dialog modal-dialog-scrollable modal-dialog-centered ${customModalDialog ? customModalDialog : 'modal-lg'}`}
        >
      ...
      {onShow && <div className="modal-backdrop fade show"></div>}
    </Fragment>
  );
}
